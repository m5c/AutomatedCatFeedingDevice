...
class Clock:
    ...
    def __init__(self, start_time_seconds: int, clock_observer: ClockSubscriber):
        """
        :param start_time_seconds: as default time to put on queue.
        :param clock_observer: as observer for value changes.
        """
        ...
        self.__clock_observer = clock_observer

        # initial update to subscriber
        if clock_observer is not None:
            clock_observer.update_time(start_time_seconds)
    ...
    def start_clock(self):
        ...
        self.__running = True
        count_down_thread = Thread(target=self.count_down)
